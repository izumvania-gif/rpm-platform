...
**Information architecture:** top nav (`components/shared/site-nav.tsx`) is 5 main tabs — Продукты, Сегменты, Исследования, Маркетинг (renamed from RTB; route is `/marketing`, the Prisma model is still called `RTB`), JTBD. Продукты and Исследования have a hover/focus dropdown sub-menu (Фичи; Гипотезы + Разговоры respectively). Competitors has no nav entry at all — `/competitors` still exists as a route but is only linked from within a product's detail page, never from top nav. JTBD has its own tab (not nested under anything) with an in-page "Список"/"Граф" tab switcher (`components/shared/jtbd-view-tabs.tsx`) between the plain list and the graph canvas.
