...
**Planned information architecture** (not yet implemented — see growth plan): top nav collapses from today's 9 flat links to 5 main tabs — Продукты, Сегменты, Исследования, Маркетинг (renamed from RTB), JTBD. Competitors and Features become sub-tabs reached through the Products area (Competitors specifically becomes reachable only from within a given product, not as a global cross-product list). Hypotheses and Conversations become sub-tabs reached through the Research area. JTBD gets its own top-level tab (it's the platform's methodological core, not a sub-item of anything else), with "Список" (the plain list) and "Граф" (the graph canvas) as its two sub-tabs, side by side for quick access to either view.
