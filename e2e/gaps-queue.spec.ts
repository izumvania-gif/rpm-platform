...
test('a hypothesis written today is not yet treated as stuck', async ({ page }) => {
  ...
  await selectOptionRobust(page, page.getByLabel('Продукт', { exact: true }), productName)
  await page.getByRole('button', { name: 'Создать' }).click()
  // С фазы 24 плана 2.4 createHypothesis ведёт на карточку, как сегмент и
  // фича. `c[a-z0-9]{10,}$`, а не `[^/]+$`: последнее совпадало с
  // `/hypotheses/new` ещё до отправки формы, и тест шёл дальше, ничего не
  // дождавшись.
