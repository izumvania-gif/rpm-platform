...
**Версия:** 1.3 (обновлено по факту реализации)  
...
**Готово:** Фаза 0 (инфраструктура), Фаза 2 (Продукты), Фаза 3 (Исследования), Фаза 4 (Сегменты), Фаза 5 (связи между модулями — частично), Фаза 6 (полировка и тестирование — частично, без toast), а также **JTBD, Гипотезы и Разговоры (CustDev) из P1** — реализованы раньше срока вместе с P0, т.к. были явно запрошены до завершения Фазы 6.
...
4. **Фаза 6 (полировка) реализована частично.** Есть loading-skeleton, pending-состояние кнопок, `error.tsx`/`not-found.tsx`, адаптивность проверена и один найденный баг исправлен. Toast-уведомлений и кастомного `ConfirmDialog` всё ещё нет — см. раздел "Фаза 6" ниже для полного разбора и списка найденных/исправленных при тестировании багов.
...
- ✅ Обработка ошибок и валидация — сообщение над формой (уже было) + `app/error.tsx` для непредвиденных ошибок
- ✅ Loading states и skeleton screens — `loading.tsx` на всех 6 списочных страницах (`components/shared/loading-skeleton.tsx`) + pending-состояние кнопок отправки форм (`components/shared/submit-button.tsx`, использует `useFormStatus`)
- ✅ Пустые состояния (empty states) — уже были у каждого списка ("Продуктов пока нет" и т.п.)
- ✅ Подтверждение удаления — уже было, нативный `confirm()`
- ❌ Toast уведомления — не реализовано; redirect на страницу сущности после успешного сохранения служит implicit-подтверждением
- ✅ Адаптивная верстка — навигация (`SiteNav`) была протестирована и **исправлена**: на узких экранах шесть пунктов меню наезжали на лого без `min-w-0`/`overflow-x-auto`, из-за чего строка вылезала за экран
- ✅ Ручное тестирование всех функций — прогнано через Playwright по всем 6 модулям (CRUD + отмена по Cancel-кнопке не применимо, каскадное удаление, дубликаты slug, границы валидации)

**Компоненты (по факту):**

- ✅ `app/error.tsx` — обработка ошибок (route-level Error Boundary, не отдельный переиспользуемый компонент)
- ✅ `LoadingSkeleton` — используется через `loading.tsx` файлы
- ❌ ~~`EmptyState`~~ — не выделен отдельным компонентом, текст пишется инлайн на каждой странице (3+ одинаковых строк — осознанно не абстрагировано)
- ❌ ~~`ConfirmDialog`~~ — нативный `confirm()` вместо кастомного диалога
- ❌ ~~`Toast`~~ — не реализовано
- ✅ `SubmitButton` — не было в исходном плане, добавлен для pending-состояния при отправке форм

**Тестирование (что реально проверено):**

- ✅ Создание, редактирование, удаление всех сущностей (все 6 модулей)
- ✅ Валидация форм — включая found bugs: пустая форма (client-side blocked), дубликат slug (server-side error), `audienceShare` вне диапазона 0–100 (client-side blocked через `max`)
- ✅ Каскадное удаление — удаление продукта корректно убирает связанные сегмент/исследование/JTBD/гипотезу/разговор (проверено через 404 на все пять после удаления родителя)
- ✅ Проверка на разных разрешениях экрана — mobile (375px), tablet (640px), desktop (1280px), без горизontального переполнения после исправления `SiteNav`

**Найденные и исправленные баги в процессе тестирования:**

1. **Foreign key crash** при создании JTBD/Hypothesis/Conversation с пустым необязательным полем (сегмент/исследование/JTBD): `z.string().optional().or(z.literal('').transform(() => undefined))` не срабатывал, т.к. `.optional()` считает `''` валидным присутствующим значением и никогда не доходит до fallback-ветки — форма падала с `PrismaClientKnownRequestError` (500). Исправлено через `lib/validation.ts` (`optionalString`/`optionalNumber` с `z.preprocess`), применено во всех 4 экшенах, где встречался этот паттерн.
2. **Тихая порча данных**: та же причина у числовых полей — `z.coerce.number().optional()` на `''` даёт `0` (т.к. `Number('') === 0` в JS), а не `undefined`. `audienceShare` и `priority`, оставленные пустыми, тихо сохранялись как `0`. Исправлено тем же способом.
3. **Респонсив**: навигация с 6 пунктами переполняла контейнер на узких экранах из-за отсутствующего `min-w-0` на flex-дочернем элементе. Исправлено (`overflow-x-auto` + `min-w-0` на `<nav>`).

**Известное ограничение (не баг, не исправлялось):** `notFound()` на динамически стримящихся страницах не устанавливает HTTP-статус 404 (Next.js 14 отдаёт 200) — сама UI-страница "не найдено" рендерится корректно, но это ограничение потоковой отдачи ответа в App Router, не специфичное для этого кода. Для внутреннего инструмента без публичной SEO-выдачи не критично.
...
### Milestone 5: MVP готов к использованию — ✅ ДОСТИГНУТ (без toast-уведомлений)

**Критерии:**

- ✅ Все модули P0 работают (плюс JTBD/Гипотезы/Разговоры из P1)
- ✅ Связи между модулями работают
- ✅ Обработка ошибок — сообщения над формой, `confirm()` перед удалением, `app/error.tsx` для непредвиденных ошибок, `app/not-found.tsx` для 404; toast всё ещё не реализован
- ✅ UI отполирован — loading-скелетоны на списках, pending-состояние кнопок отправки, найденные и исправленные проблемы валидации (см. раздел "Фаза 6" выше)
- ✅ Адаптивная верстка — проверена на 375/640/1280px, найден и исправлен реальный баг переполнения навигации на узких экранах
- ✅ Ручное тестирование — пройдено через автоматизированный browser-flow (Playwright) по всем 6 модулям, включая граничные случаи (дубликаты, диапазоны, каскадное удаление); не по буквальным шагам сценариев раздела 7.1, но эквивалентного покрытия
